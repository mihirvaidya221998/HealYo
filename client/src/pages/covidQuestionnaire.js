...
const CovidQuestionnaire = () => {
  ...
  const handleSubmit = (event) => {
    ...
    let countSymptoms = 0;
    if (hasFever) countSymptoms++;
    if (hasCough) countSymptoms++;
    if (hasBreathShortness) countSymptoms++;
    if (hasSoreThroat) countSymptoms++;
    if (hasTasteLoss) countSymptoms++;
    if (hasSmellLoss) countSymptoms++;
    if (hasContactWithCovid) countSymptoms += 4;

    if (countSymptoms >= 4) {
      message.success(
        'Covid-19 details saved successfully. Please get a Covid test done.'
      );
    } else message.success('Covid-19 details saved successfully.');
